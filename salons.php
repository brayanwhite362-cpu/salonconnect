...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  $sid    = (int)($_POST["salon_id"] ?? 0);
  $action = $_POST["action"] ?? "";
  if ($sid > 0 && in_array($action, ["disable","enable"], true)) {
    $new = $action === "disable" ? "inactive" : "active";
    $st  = $conn->prepare("UPDATE salons SET status=? WHERE id=?");
    $st->bind_param("si", $new, $sid);
    $st->execute();
  }
...
$res = $conn->query("SELECT id, name, address, phone, status, owner_id FROM salons ORDER BY id ASC");
?>
<!doctype html>
<html lang="en">
<head>
  ...
  <title>Admin · Salons | SalonConnect</title>
...
<div class="dashboard-container">
  ...
  <aside class="dashboard-sidebar">
    ...
    <nav class="nav glass-soft">
      ...
      <a class="active" href="salons.php"><span class="material-symbols-rounded">storefront</span><span>Salons</span></a>
  ...
  <main class="dashboard-main">
    ...
    <header class="header glass" style="overflow: visible !important; position: relative; z-index: 1000;">
      <div>
        <div class="header-title">Salons</div>
        <div class="muted" style="font-size:13px;">Enable or disable salon listings.</div>
      </div>
          ...
          <table>
            ...
            <?php while($s = $res->fetch_assoc()): ?>
              <tr>
                ...
                <td class="muted"><?= htmlspecialchars($s["address"]) ?></td>
                <td class="muted"><?= htmlspecialchars($s["phone"]) ?></td>
                <td class="muted"><?= $s["owner_id"] ? "Claimed" : "Unclaimed" ?></td>
                <td>
                  <?php if ($s["status"] === "active"): ?>
                    <span class="chip ok"><span class="material-symbols-rounded" style="font-size:16px;">check_circle</span>Active</span>
                  <?php else: ?>
                    <span class="chip bad"><span class="material-symbols-rounded" style="font-size:16px;">block</span>Inactive</span>
                  <?php endif; ?>
                </td>
                <td>
                  <div class="actions">
                    <form method="post">
                      ...
                      <?php elseif ($s["status"] === "inactive"): ?>
                        ...
                        <button class="btn" type="submit"><span class="material-symbols-rounded">check_circle</span> Enable</button>
                      <?php endif; ?>
                    </form>
                  </div>
                </td>
